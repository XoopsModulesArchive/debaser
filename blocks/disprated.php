...
//  ------------------------------------------------------------------------ //
	...
	function b_debaser_disprated_show($options) {
		...
		$result = $xoopsDB->query("SELECT xfid, filename, title, artist, genreid, approved, fileext, rating, uid, haslofi FROM ".$xoopsDB->prefix('debaser_files')." WHERE approved = 1 AND rating > 0 ORDER BY rating DESC LIMIT 1");
