...
//  ------------------------------------------------------------------------ //
	...
	function b_debaser_dispdown_show($options) {
		...
		$result = $xoopsDB->query("SELECT xfid, filename, title, artist, genreid, approved, fileext, hits, uid, haslofi FROM ".$xoopsDB->prefix('debaser_files')." WHERE approved = 1 AND hits > 0 ORDER BY hits DESC LIMIT 1");
				...
				if ($results = array_search_ext($fileextstack, $myrow['fileext'], false)) {
  				foreach($results as $res) {
					...
					$path = getthedir($myrow['uid']);
						...
						$imagebar .= '<a href="'.DEBASER_UUP.'/'.$path.$lofi.$myrow['filename'].'" target="_blank"><img src="'.DEBASER_UIMG.'/playericons/'.$iconstack[$thekey].'" alt="'.$namestack[$thekey].'" title="'.$namestack[$thekey].'" /></a> ';
					}
  				}
			}
