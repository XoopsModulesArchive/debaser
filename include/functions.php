...
//  ------------------------------------------------------------------------ //
    ...
    global $xoopsDB, $xoopsModule, $xoopsConfig, $xoopsModuleConfig;
        ...
        echo "<td style='width: 55%; font-size: 10px; text-align: right; color: #2F5376; padding: 0 6px; line-height: 18px;'><b>".$xoopsModule->name()._AM_DEBASER_MODADMIN . '</b> '
         . $breadcrumb . '</td>';
        echo '</tr></table>';
        echo '</div>';
 ...
 **/
    ...
    {
        global $xoopsDB;
        $preselectsql = 'SELECT d.xfid, d.fileext, e.mime_ext, e.mime_preselect, f.xpid, f.html_code, f.height, f.width, f.autostart FROM ' . $xoopsDB->prefix('debaser_files') . ' d, '
                    . $xoopsDB->prefix('debaser_mimetypes') . ' e, '
                    . $xoopsDB->prefix('debaser_player') . ' f WHERE d.xfid = '
                    . (int)$fileid . ' AND d.fileext = e.mime_ext AND e.mime_preselect = f.xpid ';
 ...
 **/
    function getTotalDebaserItems($sel_id = 0)
    {
        ...
        $arr = array();
        $query = 'SELECT d.genreid, d.genretitle, t.xfid, t.genre, t.approved from ' . $xoopsDB->prefix('debaser_genre') . ' d, '
             . $xoopsDB->prefix('debaser_files') . ' t WHERE d.genretitle = t.genre AND t.approved = 1 ';
        if ($sel_id) {
        ...
        while (list($genreid, $genretitle, $xfid, $genre, $approved) = $xoopsDB -> fetchRow($result)) {
            if ($xoopsModuleConfig['usecatperm'] == 1) {
                if ($gpermHandler -> checkRight('DebaserFilePerm', $xfid, $groups, $xoopsModule -> getVar('mid'))) {
                    $count++;
                }
            } else {
                $count++;
